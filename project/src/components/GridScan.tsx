...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  const onScanPhaseRef = useRef(onScanPhase);
  const scanDurationRef = useRef(scanDuration);
  const scanDelayRef = useRef(scanDelay);
  // keep refs in sync every render
  onScanPhaseRef.current = onScanPhase;
  scanDurationRef.current = scanDuration;
  scanDelayRef.current = scanDelay;
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // Compute scan phase and expose via callback
      if (onScanPhaseRef.current) {
        const _dur = Math.max(0.05, scanDurationRef.current ?? 2.0);
        const _del = Math.max(0.0, scanDelayRef.current ?? 2.0);
        const _cycle = _dur + _del;
        const _tCycle = (now / 1000) % _cycle;
        const _scanPhase = Math.max(0, Math.min(1, (_tCycle - _del) / _dur));
        onScanPhaseRef.current(_scanPhase);
      }
...
function srgbColor(hex: string) {
  // O Three.js moderno já trata a cor em sRGB e converte para Linear automaticamente
